...
<div className='overflow-auto bg-slate-950 text-white rounded-sm p-2 break-words'>
    ...
    return (
        <main className='h-screen w-screen flex'>
            {/* LEFT CHAT PANEL */}
<section className="left relative flex flex-col h-screen min-w-96 max-w-96 bg-slate-300 overflow-hidden">
                ...
                <div className="conversation-area pt-14 pb-10 flex-grow flex flex-col relative">
                    ...
                    <div
                      ref={messageBox}
                           className="message-box p-1 flex-grow flex flex-col gap-1 overflow-y-auto overflow-x-hidden max-h-full scrollbar-hide">


                        {messages.map((msg, index) => (
                            <div
                                key={msg._id || index}
                                className={`${msg.sender._id === 'ai' ? 'max-w-[32rem]' : 'max-w-52'}
                            ...
                            {openFiles.map(file => (
                                <button
                                    ...
                                    className={`open-file cursor-pointer p-2 px-4 flex items-center gap-2 
                                        bg-slate-300 ${currentFile === file ? 'bg-slate-400' : ''}`}
                                >
                                    <p className='font-semibold text-lg'>{file}</p>
                                </button>
                            ))}
                            ...
                            <button
                                onClick={async () => {
                                    try {
                                        ...
                                        const temp = await webContainer?.spawn("npm", ["start"]);
                                        temp.output.pipeTo(new WritableStream({
                                            write(chunk) { console.log(chunk) }
                                        }))
                                        ...
                                        console.warn("WebContainer run failed:", err)
                                    }
                                }}
